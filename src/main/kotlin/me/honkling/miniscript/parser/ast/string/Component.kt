...
abstract class Component(location: Location, parent: ComplexString?) : Value<String>(location, parent) {
    class Plain(private val value: String, location: Location, parent: ComplexString?) : Component(location, parent) {
        override fun get() = value to ExecutionResult.ContinueExecution
    }

    class Variable(private val name: String, location: Location, parent: ComplexString?) : Component(location, parent) {
        override fun get(): Pair<String, ExecutionResult> {
            val value = (parent!!.getSymbol(name)?.toString()
                ?: throw MiniScriptException.RuntimeError("Symbol '$name' doesn't exist.", this))

            return value to ExecutionResult.ContinueExecution
        }
    }

    class Expression(private val expression: MSExpression<*>, location: Location, parent: ComplexString?) : Component(location, parent) {
        override fun get(): Pair<String, ExecutionResult> {
